...
# Names=['TransTEE']
# Algs = ['TransTEE']
Algs = ['cross_species','GSM','Norman2019_prep','pachter']
# x = ['0','1','2']
style_dict = {
...
for a in range(len(Algs)):
    for b in range(len(x)):
        ...
        path_ab = "saved_models/TransTEE/{}/tr10/trb2.0/dob{}/p00.5result.json".format(Algs[a], x[b])
...
def draw(means, stds, name):
    ...
    plt.legend(('cross_species','GSM','Norman2019_prep','pachter'), frameon=False, loc='best')
    pdf = PdfPages(name)
    #plt.ylim(0, 10)
    #plt.tight_layout()
    pdf.savefig()
